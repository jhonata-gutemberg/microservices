package dev.gutemberg.device.management.api.config.client;

import dev.gutemberg.device.management.api.client.SensorMonitoringClient;
import dev.gutemberg.device.management.api.client.SensorMonitoringRestClientFactory;
import lombok.RequiredArgsConstructor;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.web.client.RestClient;
import org.springframework.web.client.support.RestClientAdapter;
import org.springframework.web.service.invoker.HttpServiceProxyFactory;

@Configuration
@RequiredArgsConstructor
public class RestClientConfig {
    private final SensorMonitoringRestClientFactory sensorMonitoringRestClientFactory;

    @Bean
    SensorMonitoringClient sensorMonitoringClient() {
        RestClient restClient = sensorMonitoringRestClientFactory.build();
        RestClientAdapter adapter = RestClientAdapter.create(restClient);
        HttpServiceProxyFactory proxyFactory = HttpServiceProxyFactory.builderFor(adapter).build();
        return proxyFactory.createClient(SensorMonitoringClient.class);
    }
}
